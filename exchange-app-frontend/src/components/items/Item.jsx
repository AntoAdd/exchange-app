...
const Item = ({
  ...
  let className = "w-100 h-100 position-relative";

  if (isCardTop) {
    className += " rounded-top overflow-hidden";
  } else {
    className += " card border border-0 rounded shadow";
  }
  ...
  let itemDisplayClass = "d-flex flex-column";
  if (!isCardTop) {
    itemDisplayClass += " h-100";
  }
  ...
  return (
    <div className={className} onClick={(e) => handleSelection(e, id)}>
      ...
      <div className={itemDisplayClass}>
